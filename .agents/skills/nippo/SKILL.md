...
- Treat one token matching `today`, `yesterday`, `this-week`, `last-week`, `week-before-last`, `this-month`, `last-month`, or `month-before-last` as a period selector and pass it through to `--period`. Do not also pass `--days` in that case; `--period` overrides it. For `daily`, use the period's representative date (today for `today`, the previous day for `yesterday`, the range's end date for weekly/monthly windows) as the output filename date.
